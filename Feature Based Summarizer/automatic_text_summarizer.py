...
sentences_with_scores = tf_idf_vectorize(sentence_mapper)

original_sentences = list(sentence_mapper.keys())
columns = ['tf-score', 'length-score', 'position-score', 'paragraph-score', 'cue-words-score', 'paragraph-id']
...
for index, row in vector_space.iterrows():
    vector_space.loc[index, 'paragraph-score'] = keyword_mapper[row['paragraph-id']]

num_cue_words = len(cue_words)
...
vector_space['sentence-score'] = vector_space.apply(lambda row: (row['tf-score'] + row['length-score'] + row['position-score'] + row['cue-words-score']) * row['paragraph-score'], axis = 1)
